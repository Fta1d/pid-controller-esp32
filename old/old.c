...
uint16_t speed_to_duty(uint8_t speed) {
    ...
    uint16_t duty = MIN_PWM_DUTY + (uint16_t)(speed * duty_range);
...
void parse_analog_command(const char *input) {
    ...
    if (strncmp(input, "SET", 3) == 0) {
        ...
        char axis = *ptr;
        if (axis != 'X' && axis != 'Y') {
            ESP_LOGW("parse", "Invalid axis: %c", axis);
            return;
        }
        ptr++; 
        ...
        if (endptr == ptr) {
            ESP_LOGW("parse", "Invalid frequency value");
            return;
        }

        if (normalized_freq < 0.0f || normalized_freq > 1.0f) {
            ESP_LOGW("parse", "Frequency out of range: %.3f", normalized_freq);
            return;
        }
        ...
        if (direction != 0 && direction != 1) {
            ESP_LOGW("parse", "Invalid direction: %d", direction);
            return;
        }
        ...
        if (axis == 'X') {
            ...
            ESP_LOGI("parse", "X axis: speed=%.3f, direction=%d", normalized_freq, direction);
        } else if (axis == 'Y') {
            analog_state.y_active = (normalized_freq > 0.0f); 
            analog_state.y_speed = normalized_freq; 
            analog_state.y_direction = (direction == 1);
            ESP_LOGI("parse", "Y axis: speed=%.3f, direction=%d", normalized_freq, direction);
        }
