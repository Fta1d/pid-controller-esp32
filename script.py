...
Uses new SETX/Y normalized_freq dir command format
"""
...
class TurretController:
    def __init__(self):
        ...
        # Last sent commands to avoid duplicates
        self.last_x_command = ""
        self.last_y_command = ""
    ...
    def send_axis_command(self, axis, normalized_freq, direction):
        """Send SET command for specific axis if it changed"""
        command = f"SET{axis} {normalized_freq:.3f} {direction}"
        
        if axis == 'X':
            if command != self.last_x_command:
                self.send_to_esp32(command)
                self.last_x_command = command
        elif axis == 'Y':
            if command != self.last_y_command:
                self.send_to_esp32(command)
                self.last_y_command = command
    
    def keyboard_mode(self):
        """Keyboard control mode with new SET commands"""
        print("KEYBOARD MODE")
        ...
        while self.running and not self.gamepad_mode:
            ...
            x_active = self.key_states['left'] or self.key_states['right']
            if x_active != last_x_state or (x_active and 
                ((self.key_states['left'] and not self.key_states['right']) != 
                 (last_x_state and self.key_states['left']))):
                
                if self.key_states['left'] and not self.key_states['right']:
                    self.send_axis_command('X', 1.0, 0)  # Left = direction 0
                elif self.key_states['right'] and not self.key_states['left']:
                    self.send_axis_command('X', 1.0, 1)  # Right = direction 1
                else:
                    self.send_axis_command('X', 0.0, 0)  # Stop
                
                last_x_state = x_active
            
            # Check vertical movement
            y_active = self.key_states['up'] or self.key_states['down']
            if y_active != last_y_state or (y_active and 
                ((self.key_states['up'] and not self.key_states['down']) != 
                 (last_y_state and self.key_states['up']))):
                
                if self.key_states['up'] and not self.key_states['down']:
                    self.send_axis_command('Y', 1.0, 1)  # Up = direction 1
                elif self.key_states['down'] and not self.key_states['up']:
                    self.send_axis_command('Y', 1.0, 0)  # Down = direction 0
                else:
                    self.send_axis_command('Y', 0.0, 0)  # Stop
                
                last_y_state = y_active
            
            time.sleep(0.03)  # Faster polling for better response
    
    def gamepad_mode_loop(self):
        """Gamepad control mode with new SET commands"""
        print("GAMEPAD MODE")
        print("Left stick - movement, button 0 - shoot, button 1 - return to keyboard")
        
        deadzone = 0.2
        last_send_time = 0
        send_interval = 0.02  # Send max every 50ms (20 FPS)
            ...
            try:
                ...
                if current_time - last_send_time >= send_interval:
                    if self.gamepad:
                        ...
                        if abs(x) > deadzone:
                            normalized_freq = abs(x)  # Already normalized 0-1
                            direction = 1 if x > 0 else 0  # Right=1, Left=0
                            self.send_axis_command('X', normalized_freq, direction)
                        else:
                            self.send_axis_command('X', 0.0, 0)  # Stop X
                        
                        # Process Y axis (vertical movement)
                        if abs(y) > deadzone:
                            normalized_freq = abs(y)  # Already normalized 0-1
                            direction = 0 if y > 0 else 1  # Down=0, Up=1 (inverted because stick up is negative)
                            self.send_axis_command('Y', normalized_freq, direction)
                        else:
                            self.send_axis_command('Y', 0.0, 0)  # Stop Y
        ...
        try:
            ...
            if not self.gamepad_mode:
                if key == keyboard.Key.up:
                    ...
                    # Для старих команд налаштування (якщо потрібно)
    ...
    def run(self):
        """Main program loop"""
        print("TURRET CONTROL SYSTEM (NEW SET COMMANDS)")
        print("=" * 40)
...
def main():
    """Main function"""
    print("Starting turret control system with new SET commands...")
